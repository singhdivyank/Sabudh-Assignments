...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        #        Initialize weight with the `nn.init.xavier_uniform_` function and bias with `nn.init.uniform_`
        #        with default parameters.
        #  Note: Trainable variables are declared as `nn.Parameter` which is a commonly used API to include a tensor
        #  into a computational graph to support updating w.r.t its gradient.
        #       Here, we use Xavier Uniform Initialization for our Weight initialization.
        #       It has been shown empirically, that this provides better initial weights for training networks
    ...
    def embedding_lookup(self, w):
        ...
        #  Pytorch has some useful APIs for you, and you can use either one in this problem (except nn.Embedding).
    ...
    def forward(self, w):
        ...
        #  Note: We do not apply the softmax to the logits here, because the loss function (torch.nn.CrossEntropyLoss)
        #  applies it more efficiently.
        #  Please see the following docs for support:
