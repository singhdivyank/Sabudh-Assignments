...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            self.dependencies.append((self.stack[-1], self.stack.pop(-2)))  # dependent = self.stack.pop(-2)
        elif transition == 'RA':
            self.dependencies.append((self.stack[-1], self.stack.pop()))  # dependent = self.stack.pop()
        else:
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses):
        parses = unfinished_parses[:batch_size]  # minibatch
        transition = model.predict(parses)
        for transition, pp in zip(parses, transition):
