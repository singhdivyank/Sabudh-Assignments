...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        @param source: list of source sentence tokens -- List[List[str]]
        @param target: list of target sentence tokens, wrapped by `<s>` and `</s>` -- List[List[str]]
    ...
    def encode(self, source_padded: torch.Tensor,
        ...
        @param source_padded: Tensor of padded source sentences with shape (src_len, b, max_word_length), where
                                        b = batch_size, src_len = maximum source sentence length. Note that
                                       these have already been sorted in order of longest to shortest sentence -- Tensor
        @param source_lengths: List of actual lengths for each of the source sentences in the batch -- Lst[int]
        ...
        enc_hiddens, _ = pack_padded_sequence(enc_hiddens)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        @param enc_hiddens: Hidden states (b, src_len, h*2), where
                                     b = batch size, src_len = maximum source sentence length,
                                      h = hidden size -- Tensor
        @param enc_masks: Tensor of sentence masks (b, src_len), where
                                     b = batch size, src_len = maximum source sentence length -- Tensor
        @param dec_init_state: Initial state and cell for decoder -- tuple(Tensor, Tensor)
        @param target_padded: Gold-standard padded target sentences (tgt_len, b, max_word_length), where
                                       tgt_len = maximum target sentence length, b = batch size -- Tensor
        @returns combined_outputs: combined output tensor  (tgt_len, b,  h), where
                                        tgt_len = maximum target sentence length,
                                        b = batch_size,  h = hidden size -- Tensor
        """
        ...
        for Y_t in torch.split(y, 1, dim=0):
            squeeze = torch.squeeze(Y_t)
            Ybar_t = torch.cat((squeeze, o_prev), dim=1)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        @param Ybar_t: Concatenated Tensor of [Y_t o_prev], with shape (b, e + h). The input for the decoder,
                                where b = batch size, e = embedding size, h = hidden size -- Tensor
        ...
        @param enc_hiddens: Encoder hidden states Tensor, with shape (b, src_len, h * 2), where b = batch size,
                                    src_len = maximum source length, h = hidden size -- Tensor
        @param enc_hiddens_proj: Encoder hidden states Tensor, projected from (h * 2) to h. Tensor is with shape (b, src_len, h),
                                    where b = batch size, src_len = maximum source length,
                                    h = hidden size -- Tensor
        @param enc_masks: Tensor of sentence masks shape (b, src_len),
                                    where b = batch size, src_len is maximum source length -- Tensor
        @returns dec_state: Tuple of tensors both shape (b, h), where b = batch size, h = hidden size.
                First tensor is decoder's new hidden state, second tensor is decoder's new cell
                -- tuple (Tensor, Tensor)
        @returns combined_output: Combined output Tensor at timestep t, shape (b, h),
                                  where b = batch size, h = hidden size -- Tensor
        @returns e_t: Tensor of shape (b, src_len). It is attention scores distribution.
                                Note: You will not use this outside of this function.
                                      We are simply returning this value so that we can sanity check
                                      your implementation -- Tensor
        """
        ...
        a_t_view = (alpha_t.size(0), 1, alpha_t.size(1))
        a_t = torch.bmm(alpha_t.view(*a_t_view), enc_hiddens).squeeze(1)
    ...
    def generate_sent_masks(self, enc_hiddens: torch.Tensor,
        ...
        @param enc_hiddens: encodings of shape (b, src_len, 2*h), where b = batch size,
                                     src_len = max source length, h = hidden size -- Tensor
        @param source_lengths: List of actual lengths for each of the sentences in the batch -- List[int]

        @returns enc_masks: Tensor of sentence masks of shape (b, src_len),
                                    where src_len = max source length, h = hidden size -- Tensor
        """
    ...
    def beam_search(self, src_sent: List[str], beam_size: int = 5,
        ...
        @param src_sent: a single source sentence (words) -- List[str]
        @param beam_size: beam size -- int
        @param max_decoding_time_step: maximum number of time steps to unroll the decoding RNN -- int
        @returns hypotheses: a list of hypothesis, each hypothesis has two fields -- List[Hypothesis]:
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            exp_src_encodings_att_linear = src_encodings_att_linear.\
                expand(hyp_num, src_encodings_att_linear.size(1), src_encodings_att_linear.size(2))
            y_tm1 = self.vocab.tgt.to_input_tensor_char(list([hyp[-1]] for hyp in hypotheses),
                                                        device=self.device)
            ...
            contiuating_hyp_scores = (hyp_scores.unsqueeze(1).expand_as(log_p_t) + log_p_t)\
                .view(-1)
            top_cand_hyp_scores, top_cand_hyp_pos = torch.topk(contiuating_hyp_scores,
                                                               k=live_hyp_num)
            ...
            if len(decoderStatesForUNKsHere) > 0 and self.charDecoder is not None:  # decode UNKs
                ...
                assert len(decodedWords) == decoderStatesForUNKsHere.size()[0],\
                    "Incorrect number of decoded words"
    ...
    def load(model_path: str, no_char_decoder=False):
        ...
        @param model_path: path to model -- str
        """
    ...
    def save(self, path: str):
        ...
        @param path: path to the model -- str
        """
