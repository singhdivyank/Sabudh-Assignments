...
def question_2a_sanity_check(decoder, char_vocab):
    ...
    assert(list(logits.size()) == logits_expected_size), "Logits shape is incorrect:\n " \
                                                         "it should be {} but is:\n{}" \
                                                         "".format(logits_expected_size,
                                                                   list(logits.size()))
    assert(list(dec_hidden1.size()) == dec_hidden_expected_size),\
        "Decoder hidden state shape is incorrect:\n it should be " \
        "{} but is: {}".format(dec_hidden_expected_size, list(dec_hidden1.size()))
    assert(list(dec_hidden2.size()) == dec_hidden_expected_size), "Decoder hidden state shape is " \
                                                                  "incorrect:\n it should be {} " \
                                                                  "but is: {}".format(dec_hidden_expected_size,
                                                                                      list(dec_hidden2.
                                                                                           size()))
...
def question_2b_sanity_check(decoder):
    ...
    assert(list(loss.size()) == []), "Loss should be a scalar but its " \
                                     "shape is: {}".format(list(loss.size()))
...
def question_2c_sanity_check(decoder):
    ...
    assert(len(decodedWords) == BATCH_SIZE), "Length of decodedWords " \
                                             "should be {} but is: {}".format(BATCH_SIZE,
                                                                              len(decodedWords))
...
def main():
    ...
    assert(torch.__version__ >= "1.0.0"), "Please update your installation of PyTorch. " \
                                          "You have {} and you should " \
                                          "have version 1.0.0".format(torch.__version__)
    ...
    # Create NMT Model
    model = NMT(word_embed_size=EMBED_SIZE, hidden_size=HIDDEN_SIZE,
                dropout_rate=DROPOUT_RATE, vocab=vocab)

    char_vocab = DummyVocab()

    # Initialize CharDecoder
    decoder = CharDecoder(hidden_size=HIDDEN_SIZE, char_embedding_size=EMBED_SIZE,
