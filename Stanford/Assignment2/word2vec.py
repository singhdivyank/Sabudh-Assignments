...
def negSamplingLossAndGradient(
    ...
    ovector = outsideVectors[outsideWordIdx]
    nvector = outsideVectors[negSampleWordIndices]

    # Please use your implementation of sigmoid in here.

    value_outside = ovector.dot(centerWordVec)
    value_negative = nvector.dot(centerWordVec)

    p = sigmoid(value_outside)
    n = sigmoid(- value_negative)

    loss = - (np.log(p) + np.sum(np.log(n)))
    gradCenterVec = (p - 1) * ovector + np.sum((1 - n)[:, np.newaxis] * nvector, axis=0)

    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] = (p - 1) * centerWordVec
    for i, neg_index in enumerate(negSampleWordIndices):
        gradOutsideVecs[neg_index] += (1 - n[i]) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
             centerWordVectors, outsideVectors, dataset, word2vecLossAndGradient = naiveSoftmaxLossAndGradient):
